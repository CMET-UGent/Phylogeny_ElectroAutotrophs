...
```{r liao2018prepro, engine='bash',eval=FALSE}
...
```{r liaoclean,warning=FALSE,cache=TRUE}
...
```{r liao2018preproraw, engine='bash',eval=FALSE}
<!-- [NOTE]: Setting random seed to 19760620. -->
...
```{r liaoraw,warning=FALSE,cache=TRUE,fig.cap=c("stacked bargraph of read count of top 24 OTUs, colored by classifiation for the raw data")}
...
```{r milnerfiltering, warning=FALSE,cache=TRUE,fig.cap=c("Sequence length distribution for Milner et al. 2016")}
...
```{r strycharzfiltering,warning=FALSE,cache=TRUE,fig.cap=c("Sequence length distribution for Strycharz-Glaven et al. 2013")}
...
```{r lqmfilterfasta,warning=FALSE,cache=TRUE}
...
```{r rothballerfasta,warning=FALSE,cache=TRUE,fig.cap=c("Sequence length distribution for Rothballer et al. 2015")}
...
Using the criteria as applied to Strycharz-Glaven 2013 above, we end up with `r nrow(RBdata)` entries.
...
```{r prevoteaufasta, warning=FALSE,cache=TRUE}
...
```{r eddie1018raxml, engine='bash',eval=FALSE}
...
```{r fulllenbestreeviz, warning=FALSE, fig.cap="RAxML maximum likelihood tree with optimized branch lengths and bootstrap confidences drawn onto it (2500 bootstraps)",fig.height=9.5}
...
```{r fulllenMREviz,fig.cap="RAxML extended majority rule tree of 2500 bootstraps",fig.height=9.5}
...
```{r raxmlEPAPBOGLTPNewRefs,engine='bash',eval=FALSE}
...
```{r Fulllengthrefsistespec, fig.cap="Site-specific placement bias calculation for the full-length guide tree"}
...
```{r FLTreeepang, engine='bash',eval=FALSE}
raxmlHPC-PTHREADS-AVX -f e -s full_lengthwithOG_minimizedDNA.fasta.reduced \
 -t EPA2/RAxML_bipartitions.FulllennoOTUwOGMLtreewithBPinfo \
 -n file2 -m GTRGAMMA -u -k -T 12 \
 -w EPA2/
<!-- Warning, you specified a working directory via "-w" -->
<!-- Keep in mind that RAxML only accepts absolute path names, not relative ones! -->
<!-- Using BFGS method to optimize GTR rate parameters, to disable this specify "--no-bfgs" -->
...
<!-- Substitution Matrix: GTR -->
<!-- RAxML was called as follows: -->
<!-- raxmlHPC-PTHREADS-AVX -f e -s full_lengthwithOG_minimizedDNA.fasta.reduced -t EPA2/RAxML_bipartitions.FulllennoOTUwOGMLtreewithBPinfo -n file2 -m GTRGAMMA -u -k -T 12 -w EPA2/ -->
<!-- Model parameters (binary file format) written to: EPA2/RAxML_binaryModelParameters.file2 -->
<!-- 0.515887 -26771.831522 -->
<!-- Overall Time for Tree Evaluation 0.516161 -->
<!-- Final GAMMA  likelihood: -26771.831522 -->
<!-- Number of free parameters for AIC-TEST(BR-LEN): 200 -->
<!-- Number of free parameters for AIC-TEST(NO-BR-LEN): 9 -->
<!-- Model Parameters of Partition 0, Name: No Name Provided, Type of Data: DNA -->
<!-- alpha: 0.235409 -->
<!-- Tree-Length: 6.207057 -->
<!-- rate A <-> C: 0.797474 -->
<!-- rate A <-> G: 2.439650 -->
<!-- rate A <-> T: 1.420744 -->
<!-- rate C <-> G: 0.867885 -->
...
<!-- freq pi(G): 0.320656 -->
<!-- freq pi(T): 0.201112 -->
<!-- Final tree written to:           EPA2/RAxML_result.file2 -->
<!-- Execution Log File written to:   EPA2/RAxML_log.file2 -->



cp full_lengthwithOG_minimizedDNA.fasta.reduced  ./epangoutput/

cd epangoutput

 epa-ng --ref-msa noOTUMinimized.fasta \
 --tree EPA2/RAxML_bipartitions.FulllennoOTUwOGMLtreewithBPinfo \
 --query OTUonlyMinimized.fasta \
 --outdir /EPA2/epangoutput/ --threads 6 --model EPA2/RAxML_info.file2
<!-- INFO Selected: Output dir: EPA2/epangoutput/ -->
<!-- INFO Selected: Query file: OTUonlyMinimized.fasta -->
<!-- INFO Selected: Tree file: EPA2/RAxML_bipartitions.FulllennoOTUwOGMLtreewithBPinfo -->
<!-- INFO Selected: Reference MSA: noOTUMinimized.fasta -->
<!-- INFO Selected: Specified model file: EPA2/RAxML_info.file2 -->
<!-- INFO    Rate heterogeneity: GAMMA (4 cats, mean),  alpha: 0.235409 (user),  weights&rates: (0.25,0.00150005) (0.25,0.0563741) (0.25,0.469689) (0.25,3.47244) -->
<!--         Base frequencies (user): 0.248253 0.22998 0.320656 0.201112 -->
<!--         Substitution rates (user): 0.797474 2.43965 1.42074 0.867885 3.744 1 -->
...
<!-- INFO Output file: EPA2/epangoutput/epa_result.jplace -->
...
Next, we used [genesis](https://groups.google.com/forum/#!searchin/raxml/visualize$20jplace$20itol%7Csort:date/raxml/IildAkMRI9Q/dBFEsUKlBQAJ) with its app labelled_tree (not installed yet, only avaialble through `/usr/local/programs/genesis/bin/apps`).

```{r genesisPBLTPnewrefs, eval=FALSE, engine='bash'}
usr/local/programs/genesis/bin/apps$ ./labelled_tree\
 EPA2/epangoutput/epa_result.jplace\
 EPA2/RAxML_bipartitions.FulllennoOTUwOGMLtreewithBPinfo \
 EPA2/epangoutput/epagenesisFulllennoOTUwOGMLtreewithBPinfo.nwk
<!--                                       , -->
<!--         __    __    __    __   __     __ -->
<!--       /   ) /___) /   ) /___) (_ ` / (_ ` -->
<!--      (___/ (___  /   / (___  (__) / (__) -->
...
<!--                v0.19.0 -->
<!-- INFO Jplace file: EPA2/epangoutput/epa_result.jplace -->
...
<!-- WARN Invalid placement with proximal_length < 0.0. -->
<!-- WARN Invalid placement with proximal_length < 0.0. -->
<!-- INFO Bootstrap Tree file: EPA2/RAxML_bipartitions.FulllennoOTUwOGMLtreewithBPinfo -->
<!-- INFO Invalid pquery with sum of like_weight_ratio '1' > 1.0 at 'Otu0089_Liao2018'. -->
<!-- INFO Invalid pquery with sum of like_weight_ratio '1' > 1.0 at 'gi|508083782|gb|KC570056.1|'. -->
<!-- INFO Done. -->
